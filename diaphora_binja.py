...
class CHLILAstVisitor:
  ...
  def visit(self, node):
    ...
    try:
      ...
      if op is not None:
        op_name = getattr(op, "name", "") or str(op)
        self.primes_hash *= self.primes[_op_prime_index(op_name, self.primes)]
    except Exception:
      pass
...
# ------------------------------------------------------------------------------
def _op_prime_index(op_name, primes):
  """Stable per-IL-operation prime index for primes-product hashes.

  Hashing the operation *name* (e.g. ``"MLIL_JUMP_TO"``, ``"HLIL_CALL"``)
  rather than using the raw enum ordinal makes the index survive Binary
  Ninja releases that renumber the operation enums -- they have done so
  before, and silently. Without this, every existing pseudocode_primes /
  microcode_spp in old export DBs stops matching after a BN upgrade.
  Same fix pattern as ``_mnemonic_prime_index`` (commit 7243ce0).
  """
  if not op_name:
    return 0
  digest = md5(op_name.encode("utf-8", "replace")).digest()
  return int.from_bytes(digest[:8], "big") % len(primes)
...
class CBinjaBinDiff(diaphora.CBinDiff):
  ...
  # Token-type names that capture an instruction's opcode + addressing-mode
  # shape. Anything outside this set (immediates, addresses, floats, strings,
  # symbol/import names) is operand *value* and gets stripped when building
  # the bytes_hash signature -- analogous to IDA's offb-stripped decoded_size.
  _OPCODE_SHAPE_TOKENS = frozenset((
    "InstructionToken",
    "OpcodeToken",
    "RegisterToken",
  ...
  def _opcode_signature(self, addr, arch=None):
    """Return bytes capturing the instruction's opcode + addressing-mode
    shape, with operand-value tokens (immediates, displacements, addresses,
    string/symbol references) stripped.

    Hashing this for ``bytes_hash`` gives a feature that matches across
    binaries where the same opcode+addressing-mode appears with different
    immediates -- mirroring what IDA's ``get_decoded_instruction`` does by
    subtracting operand byte offsets from ``inst_size``. ``function_hash``
    keeps using raw bytes so the strict per-byte signature is preserved.
    Falls back to the raw bytes if tokenization fails so bytes_hash still
    discriminates rather than collapsing to a constant.
    """
    if arch is None:
      arch = self._arch_for(None)
    if arch is None:
    ...
    for t in tokens:
      try:
        ...
        ttype_name = ttype.name if ttype is not None and hasattr(ttype, "name") else str(ttype)
      except Exception:
        ttype_name = ""
      if "." in ttype_name:
        ttype_name = ttype_name.rsplit(".", 1)[-1]
      if ttype_name not in self._OPCODE_SHAPE_TOKENS:
        continue
      try:
        parts.append(str(t))
      except Exception:
  ...
  def _detect_switch_at(self, func, addr):
    """Detect a switch/jump-table at ``addr`` and return ``[case_count,
    case_values]`` or ``None``.

    The previous code hooked ``func.get_indirect_branches_at(addr)``, which
    returns *every* indirect branch (vtable/function-pointer calls,
    computed jumps, real switches) and stored target *addresses* rather
    than case *values*. That polluted the switches column with non-switches
    and made the values useless for matching across binary variations
    where switch targets are rebased but case constants aren't.

    Walk LLIL -> MLIL and only fire when the instruction is
    ``MLIL_JUMP_TO``, BN's first-class jump-table representation.
    ``targets`` is a ``dict[case_value, destination_addr]`` on BN 5.4+.
    """
    try:
    ...
    if mlil is None:
      return None
    try:
      op_name = mlil.operation.name
    except Exception:
      return None
    if op_name != "MLIL_JUMP_TO":
      return None
    targets = getattr(mlil, "targets", None)
    if not targets:
      return None
    try:
      case_values = sorted(int(k) for k in targets.keys())
    except Exception:
      return None
    if not case_values:
      return None
    return [len(case_values), case_values]
  ...
  def get_microcode(self, func):
    ...
    for i, bb in enumerate(mlil_bbs, start=1):
      ...
      for ins in insns:
        ...
        lines.append(text)
        spp *= primes[_op_prime_index(op_name, primes)]
  ...
  def get_pseudocode_primes(self, func):
    ...
    # textual pseudocode so the two stay in sync. ``hlil.instructions`` yields
    # only top-level statements; CHLILAstVisitor recurses into operands so the
    # AST coverage matches what the textual representation reflects (every
    ...
    # Cache for project hooks and any in-process consumer that looks up
    # pseudocode by address. The on-disk pseudocode column is populated
    # separately from the props tuple; these caches mirror the IDA port's
    # self.pseudo / self.pseudo_hash so hook contracts that read them work.
    ea = int(func.start)
    self.pseudo[ea] = pseudo_lines
  ...
  def run_pre_export_hook(self, ea, name):
    ...
    if self.hooks is not None and "before_export_function" in dir(self.hooks):
      if not self.hooks.before_export_function(ea, name):
        return False
    return True
  ...
  def should_skip_function(self, func, name):
    """ Check whether a function should be skipped. Returns (skip, reason).
    """
    if self.exclude_library_thunk:
      try:
        if getattr(func, "is_thunk", False):
          return True, f"Skipping thunk function {name!r}"
      except Exception:
        pass
      try:
        sym = func.symbol
        if sym is not None and sym.type == SymbolType.LibraryFunctionSymbol:
          return True, f"Skipping library function {name!r}"
      except Exception:
        pass
    ...
    if not self.ida_subs:
      if (
        ...
        return True, f"Skipping function {name!r}"
  ...
  def process_instruction(self, current_head, func, image_base, block_ea, nodes,
    ...
    opcode_sig = self._opcode_signature(current_head, arch=block_arch)
    ...
    try:
      outdegree_inc = len(list(self.bv.get_code_refs_from(current_head)))
    except Exception:
      outdegree_inc = 0

    mnemonics_spp_mult = self.primes[self._mnemonic_prime_index(mnem)] if mnem else 1

    # Name lookup: IDA scans code refs first (call/branch targets) and falls
    # back to data refs. Walking only data refs misses every
    ...
    try:
      ...
      if llil is not None:
        self._walk_llil_for_constants(llil, immediate_constants)
    except Exception:
      pass
    ...
    # Switch / jump-table detection. See _detect_switch_at for why this
    # uses MLIL_JUMP_TO instead of the broader get_indirect_branches_at.
    insn_switches = []
    sw = self._detect_switch_at(func, current_head)
    if sw is not None:
      insn_switches.append(sw)
  ...
  def process_basic_block(self, block, func, image_base, accum):
    ...
    for current_head in self._iter_instruction_addrs(block):
      ...
      if inst_data['curr_bytes']:
        # bytes_hash uses the opcode-shape signature (immediate-stripped) so
        # functions that share opcode + addressing-mode but differ in
        # immediates still match on this column. function_hash keeps the
        # raw bytes for the strict per-byte signature. Mirrors IDA's
        # decoded_size (offb-stripped) vs inst_size split. Falls back to raw
        # bytes when the tokenizer returns nothing so bytes_hash stays
        # discriminating.
        accum['bytes_hash'].append(inst_data['opcode_sig'] or inst_data['curr_bytes'])
        accum['bytes_sum'] += sum(inst_data['curr_bytes'])
        accum['function_hash'].append(inst_data['curr_bytes'])

      accum['outdegree'] += inst_data['outdegree_inc']
      accum['mnems'].append(inst_data['mnem'])

      for n in inst_data['names_to_add']:
        accum['names'].add(n)

      for ce in inst_data['insn_callees']:
        if ce not in accum['callees']:
          accum['callees'].append(ce)

      for sc in inst_data['string_constants']:
        if sc not in accum['constants']:
          accum['constants'].append(sc)

      for ic in inst_data['immediate_constants']:
        if ic not in accum['constants']:
          accum['constants'].append(ic)
      ...
      accum['switches'].extend(inst_data['insn_switches'])
    ...
    for pred_block in preds:
      ...
      try:
        accum['bb_relations'][pred_base].append(block_ea)
      except KeyError:
        accum['bb_relations'][pred_base] = [block_ea]
      accum['edges'] += 1
      accum['outdegree'] += 1

    return current_head, accum

  def build_topological_relations(self, blocks, image_base, bb_topo_num, bb_topological):
  ...
  def extract_all_features(self, f, func, current_head, data):
    """ Extract derived features (topological, assembly, pseudocode, microcode, ...).
    """
    ...
    asm, assembly_addrs = self._build_assembly_text(data['assembly'], f, data['image_base'])
    try:
    ...
    bytes_hash = md5(b"".join(data['bytes_hash'])).hexdigest() if data['bytes_hash'] else md5(b"").hexdigest()
    function_hash = md5(b"".join(data['function_hash'])).hexdigest() if data['function_hash'] else md5(b"").hexdigest()

    function_flags = self._function_flags(func)

    # HLIL -> pseudo primes + a cheap textual pseudocode. pseudocode_primes is
    # only meaningful when a textual pseudocode was actually produced; the
    # ``str(...) if pseudo else None`` gate avoids storing the sentinel "1"
    # for every BN function whose HLIL is missing/empty -- otherwise the
    # matchers treat them all as having identical AST primes and generate
    # bogus equal-pseudocode matches across unrelated functions.
    ...
    try:
      ...
      seg_rva = (current_head - int(seg[0].start)) if seg else (current_head - data['image_base'])
    except Exception:
      seg_rva = current_head - data['image_base']

    kgh_hash = self.calculate_kgh(func)
    rva = f - data['image_base']

    names = sorted(data['names'])
    export_time = str(time.monotonic() - data['export_time'])
  ...
  def build_props_list(self, name, true_name, f, data, features):
    """ Build the final 54-element properties tuple.
    """
    return (
      name,                                              # 0
      data['nodes'],                                     # 1
      ...
      features['comment'],                               # 13
      true_name,                                         # 14
      features['bytes_hash'],                            # 15
      features['pseudo'],                                # 16
      features['pseudo_lines'],                          # 17
      features['pseudo_hash1'],                          # 18
      features['pseudocode_primes'],                     # 19
      features['function_flags'],                        # 20
      features['asm'],                                   # 21
      features['proto2'],                                # 22
      features['pseudo_hash2'],                          # 23
      features['pseudo_hash3'],                          # 24
      len(features['strongly_connected']) if features['strongly_connected'] else 0,  # 25
      features['loops'],                                 # 26
      features['rva'],                                   # 27
      features['bb_topological'],                        # 28
      features['strongly_connected_spp'],                # 29
      features['clean_assembly'],                        # 30
      features['clean_pseudo'],                          # 31
      data['mnemonics_spp'],                             # 32
      data['switches'],                                  # 33
      features['function_hash'],                         # 34
      data['bytes_sum'],                                 # 35
      features['md_index'],                              # 36
      data['constants'],                                 # 37
      len(data['constants']),                            # 38
      features['seg_rva'],                               # 39
      features['assembly_addrs'],                        # 40
      features['kgh_hash'],                              # 41
      None,                                              # 42 source_file
      None,                                              # 43 userdata
      features['microcode'],                             # 44
      features['clean_microcode'],                       # 45
      features['microcode_spp'],                         # 46
      features['export_time'],                           # 47
      features['microcode_bblocks'],                     # 48
      features['microcode_bbrelations'],                 # 49
      data['callers'],                                   # 50
      data['callees'],                                   # 51
      data['basic_blocks_data'],                         # 52
      data['bb_relations'],                              # 53
    )
  ...
  def read_function(self, func):
    ...
    if not self.run_pre_export_hook(f, name):
      return False

    skip, reason = self.should_skip_function(func, name)
    if skip:
      debug_refresh(reason)
      return False
    ...
    for block in data['blocks']:
      current_head, data = self.process_basic_block(
        block, func, data['image_base'], data
      )

    data['bb_topological'] = self.build_topological_relations(
      data['blocks'], data['image_base'], data['bb_topo_num'], data['bb_topological']
    )

    features = self.extract_all_features(f, func, current_head, data)
    props_list = self.build_props_list(name, true_name, f, data, features)
    return self.run_post_export_hook(props_list)
